...
def parse_nodes_csv(nodes_csv: str) -> list[dict[str, Any]]:
    reader = csv.DictReader(StringIO(nodes_csv), restkey="__rest__", restval="")
    rows: list[dict[str, Any]] = []
    for row in reader:
        attrs_raw = (row.get("attrs_json") or row.get("attrs") or "{}").strip()
        rest = row.get("__rest__") or []
        if rest:
            attrs_raw = f"{attrs_raw},{','.join(rest)}"
...
def parse_branches_csv(branches_csv: str) -> list[dict[str, Any]]:
    reader = csv.DictReader(StringIO(branches_csv), restkey="__rest__", restval="")
    rows: list[dict[str, Any]] = []
    for row in reader:
        params_raw = (row.get("params_json") or row.get("params") or "{}").strip()
        rest = row.get("__rest__") or []
        if rest:
            params_raw = f"{params_raw},{','.join(rest)}"
        raw_id = (row.get("id") or "").strip()
        try:
