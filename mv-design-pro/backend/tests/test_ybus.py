...
def test_transformer_stamping_between_two_nodes():
    ...
    transformer = create_transformer_branch("T1", "A", "B", 10.0, 4.0, 0.1)
    graph.add_branch(transformer)

    builder = AdmittanceMatrixBuilder(graph)
    y_bus = builder.build()

    z_ohm = transformer.get_short_circuit_impedance_ohm_lv()
    y_series = 1.0 / z_ohm
...
def test_transformer_ignored_when_out_of_service():
    ...
    transformer = create_transformer_branch("T1", "A", "B", 10.0, 4.0, 0.1, in_service=False)
    graph.add_branch(transformer)

    builder = AdmittanceMatrixBuilder(graph)
    y_bus = builder.build()

    assert np.allclose(y_bus, 0.0)


def test_transformer_zk_rk_xk_pu():
    transformer = create_transformer_branch("T1", "A", "B", 10.0, 4.0, 0.1)

    z_expected = transformer.uk_percent / 100.0
    r_expected = (transformer.pk_kw / 1000.0) / transformer.rated_power_mva
    x_expected = (z_expected * z_expected - r_expected * r_expected) ** 0.5

    assert np.isclose(transformer.get_short_circuit_resistance_pu(), r_expected)
    assert np.isclose(transformer.get_short_circuit_reactance_pu(), x_expected)
    np.testing.assert_allclose(transformer.get_short_circuit_impedance_pu(), complex(r_expected, x_expected))


def test_transformer_impedance_ohm_lv():
    transformer = create_transformer_branch("T1", "A", "B", 10.0, 4.0, 0.1)

    z_base = (transformer.voltage_lv_kv ** 2) / transformer.rated_power_mva
    r_pu = (transformer.pk_kw / 1000.0) / transformer.rated_power_mva
    z_pu = transformer.uk_percent / 100.0
    x_pu = (z_pu * z_pu - r_pu * r_pu) ** 0.5
    expected = complex(r_pu * z_base, x_pu * z_base)

    np.testing.assert_allclose(transformer.get_short_circuit_impedance_ohm_lv(), expected)


def test_ybus_transformer_stamping_uses_ohm_lv():
    graph = NetworkGraph()
    graph.add_node(create_pq_node("A"))
    graph.add_node(create_pq_node("B"))
    ...
    # Stamping uses LV-side short-circuit impedance in ohms.
