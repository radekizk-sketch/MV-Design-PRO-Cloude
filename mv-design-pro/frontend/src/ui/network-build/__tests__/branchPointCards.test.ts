/**
 * Testy dla BranchPoleCard i ZksnCard — karty obiektów punktu rozgałęzienia SN.
 *
 * Weryfikuje:
 * 1. NodeTypeV1 zawiera dedykowane typy BRANCH_POLE i ZKSN_NODE
 * 2. Karty są eksportowane z barrel index
 * 3. catalogFirstRules blokuje insert_branch_pole/zksn bez catalog_ref
 */
...
describe('catalogFirstRules — catalog_ref wymagany dla branch points', () => {
  it('blokuje słup rozgałęźny bez catalog_ref', async () => {
  ...
  it('przepuszcza słup rozgałęźny z catalog_ref', async () => {
    const { validateCatalogFirst } = await import('../forms/catalogFirstRules');
    const error = validateCatalogFirst('insert_branch_pole_on_segment_sn', {
      segment_id: 'seg-001',
  ...
  it('blokuje ZKSN bez catalog_ref', async () => {
    const { validateCatalogFirst } = await import('../forms/catalogFirstRules');
  ...
  it('przepuszcza ZKSN z catalog_ref', async () => {
    const { validateCatalogFirst } = await import('../forms/catalogFirstRules');
    const error = validateCatalogFirst('insert_zksn_on_segment_sn', {
      segment_id: 'seg-001',
      catalog_ref: 'ZKSN-2P',
...
describe('BranchPointSN — model TypeScript', () => {
  it('typ BranchPointSN ma wymagane pola topologiczne', async () => {
    // Weryfikacja przez import typu — sprawdza kompilację
    const { NodeTypeV1 } = await import('../../sld/core/visualGraph');
    // Jeśli import się powiódł bez błędów TypeScript, typy są poprawne
